...
package object nspl
  ...
  implicit def compositeListRenderer2[T <: Renderable[T], T2 <: Renderable[T2], R <: RenderingContext](implicit r1: Renderer[T, R], r2: Renderer[T2, R]) =
    new Renderer[ElemList2[T, T2], R] {
      def render(ctx: R, elem: ElemList2[T, T2]): Unit = {
        elem.members.foreach(e => (e match {
