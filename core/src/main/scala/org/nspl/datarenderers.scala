        ...
        if (wX >= xAxis.min && wX <= xAxis.max && wY >= yAxis.min && wY <= yAxis.max) {
          ...
          if (valueText) {
            val tb = TextBox(
              ...
              .translate(vX, vY)
              .transform(b => tx.concat(AffineTransform.translate(0, -1 * b.h - shape.bounds.h * factorY * 0.5)))
          ...
          if (labelText) {
            val tb = TextBox(
              ...
              .translate(vX, vY)
              .transform(b => tx.concat(AffineTransform.translate(-0.2 * b.w, -1 * b.h - shape.bounds.h * factorY * 0.5)))
