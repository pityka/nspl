...
//     val t1x = (-1 * math.sqrt(-1 * p0.x * p2.x + p0.x * p3.x + p1.x * p1.x - p1.x * p2.x - p1.x * p3.x + p2.x * p2.x) - p0.x + 2 * p1.x - p2.x) / (-1 * p0.x + 3 * p1.x - 3 * p2.x + p3.x)
//     val t2x = (math.sqrt(-1 * p0.x * p2.x + p0.x * p3.x + p1.x * p1.x - p1.x * p2.x - p1.x * p3.x + p2.x * p2.x) - p0.x + 2 * p1.x - p2.x) / (-1 * p0.x + 3 * p1.x - 3 * p2.x + p3.x)
//
//     val t1y = (-1 * math.sqrt(-1 * p0.y * p2.y + p0.y * p3.y + p1.y * p1.y - p1.y * p2.y - p1.y * p3.y + p2.y * p2.y) - p0.y + 2 * p1.y - p2.y) / (-1 * p0.y + 3 * p1.y - 3 * p2.y + p3.y)
//     val t2y = (math.sqrt(-1 * p0.y * p2.y + p0.y * p3.y + p1.y * p1.y - p1.y * p2.y - p1.y * p3.y + p2.y * p2.y) - p0.y + 2 * p1.y - p2.y) / (-1 * p0.y + 3 * p1.y - 3 * p2.y + p3.y)
//
//     p0 :: p3 :: (List(t1x, t2x, t1y, t2y)
//       .filter(t => 0.0 >= t && t <= 1.0).map(t =>
//         Point(
...
//
//   }
//   def transform(tx: AffineTransform) = CubicTo(tx.transform(p1), tx.transform(p2), tx.transform(p3))
// }

case class Path(path: Seq[PathOperation]) extends Shape {
  def bounds: Bounds = {
    val current = Point(0d, 0d) +: path.map(_.last).dropRight(1)
    val extrema = path zip current flatMap (x => x._1.extremities(x._2))
