...
class ScalaTagRC private[nspl] (
    private[nspl] val elems: scala.collection.mutable.ArrayBuffer[
...
object scalatagrenderer {
  ...
  implicit val defaultGlyphMeasurer: CanvasGlyphMeasurer.type =
    CanvasGlyphMeasurer
  ...
  private[nspl] def cssColor(c: org.nspl.Color) =
  ...
  def renderToScalaTag[K <: Renderable[K]](
    ...
    val ctx = new ScalaTagRC(scala.collection.mutable.ArrayBuffer.empty)
  ...
  implicit val shapeRenderer: Renderer[ShapeElem, ScalaTagRC] =
    new Renderer[ShapeElem, ScalaTagRC] {
      def render(ctx: ScalaTagRC, elem: ShapeElem): Unit = {
        val tx =
          ctx.getTransform.applyBefore(
            elem.tx.applyBefore(elem.shape.currentTransform)
          )
              ...
              path(
                d := ops map {
                  ...
                  case PathOperation.QuadTo(Point(x2, y2), Point(x1, y1)) =>
                    s"Q$x1,$y1,$x2,$y2"
                  case PathOperation
                        .CubicTo(Point(x3, y3), Point(x1, y1), Point(x2, y2)) =>
  ...
  implicit val textRenderer: Renderer[TextBox, ScalaTagRC] =
    new Renderer[TextBox, ScalaTagRC] {
